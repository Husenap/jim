...
export default function DrawerPageContainer({
  ...
  return (
    <div className="flex h-full max-h-full min-h-full flex-1 flex-col items-center">
      {topNavbar && (
        <nav className="bg-content2 flex w-full justify-center px-2">
          <div className="w-full max-w-5xl">{topNavbar}</div>
        </nav>
      )}

      <main className="flex w-full flex-1 justify-center overflow-x-hidden overflow-y-auto overscroll-x-none px-2 py-4">
        <div className="flex w-full max-w-5xl flex-col gap-2">{children}</div>
      </main>

      {bottomNavbar ? (
        <nav className="iphone-safe-inset bg-content2 flex w-full justify-center px-2">
